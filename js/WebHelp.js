...
WebHelp = (function () {
     ...
     * @returns {promise} Promise with resolve variable pointing to an array of sequence IDs
     */
    function _getAllVisitedSequencesViaAjax(webHelpInstance){
        ...
        return jQuery.ajax({
            ...
            error: function() {
                throw new Error('Could not poll for visited sequences');
            }
        });
    }
    ...
    WebHelp.prototype.isSequenceAlreadyViewed = function (options) {
        ...
        if (!(seqName || seqId)) {
            throw new Error('Called function with no identifiers for sequence');
            return false;
        }
